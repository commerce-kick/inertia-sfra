...
- Runs against an SFCC sandbox (credentials in `dw.json`, deploy via `b2c-tools`). As of 2026-08-04 the site serves the full RefArch catalog (New Arrivals, Womens, Mens, Electronics, Top Sellers, …) — the earlier small custom demo catalog ("Sites-my-catalog": Spanish watches/accessories) is no longer assigned, and storefront copy is deliberately catalog-generic. No search refinement definitions are configured in Business Manager yet. Developers work locally with Vite dev server + HMR (`bun run dev`) against the remote sandbox.
