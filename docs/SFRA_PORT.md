...
| # | Endpoint | Kind | Page / Hook | Status | Notes |
|---|---|---|---|---|---|
| 2.1 | Cart-Show | PAGE | `Cart/Show` | DONE | fatal `inertiaMiddleware` require removed; `server.append` → `CartData` (7 modules: Cart/CartLineItem/CartBonusLineItem/CartTotals/CartDiscount/ShippingMethod + a shared `lineItemFields`). Page composes lines (ordinary / bundle / uncategorized as one compound `LineItem`), nested bonus products, applied coupons + promotions, delivery estimate, totals rail, checkout gate. Base assembled the approaching-promo sentence server-side from a resource template — the two facts (`distance`, `calloutMsg`) arrive separately and the sentence is written in the component. Dropped: `actionUrls`/`resources` bags (generated routes + inline English), `discountsHtml` (a server-rendered copy of the same array), `numOfShipments` (base shipped no multi-shipment cart), `reportingURLs` (row 10.4). Added one row base's totals lacked: subtotal. `checkoutUrl` (an absolute `URLUtils.https` URL) was carried while checkout was unported and the entry had to be a page load; 6.1 made it a page, so the bag and the flyout now visit it like any other internal link — through the relative generated helper, since an absolute URL whose host differs from the browsing host is cross-origin to an Inertia XHR. Never prefetched: base's Begin handler writes to the basket. Base never linked a cart line to its PDP — kept as base had it. Controls arrive with their own rows: coupon form 2.9, coupon remove 2.10, qty 2.6, remove 2.7, edit 2.8/2.14, shipping select 2.13, bonus chooser 2.11/2.12 |
...
| 2.4 | Cart-AddProduct | JSON | `useAddToCart` | DONE | `server.append` → `CartActionData` + `BonusOfferData`; base's whole add path (sets, bundles, option merging, bonus detection) is kept. The basket does not ride back — the cart page partial-reloads its own prop and the header count is Cart-MiniCart; what only this answer carries is the bonus offer, which exists in the difference between the basket before and after. Base put failure text in `message` with `error: true`, which the client envelope never reads — split into `errorMessage` so a failure rejects with something worth showing, and base's two candidate chooser URLs resolve to one `chooserUrl` server-side. Closes 1.6's deferrals: `ProductDetailData` gains `readyToOrder`, `selectedQuantity`, `quantities` and `options`, so `AddToBag` (PDP + quickview) holds no state — quantity and every option are resolved by the URL, exactly like variation values. **POST fields are `@formParam`, not `@queryParam`**: `getFormData` skips any key that also appears in the query string, so a POST field put in the URL never arrives, and `@queryParam` would make the generated helper demand it there. **Withdrawn since**: the option selector (Extended Warranty and its like) is no longer rendered on the PDP, the quick look or the cart's edit dialog. Choosing a value navigates to the option URL the *platform* builds (`optionModel.urlSelectOptionValue`), and base's product factory throws resolving it — `sony-ps3-bundleM?dwopt_sony-ps3-bundleM_consoleWarranty=003` answers a 500 before any port code runs, and base's own PDP reaches the same crash through Product-Variation. Lines are added with the option model's default value, which is what base applies when a request names no option; the cart line still prints whichever option the line carries, and the edit dialog still sends the line's existing option value so editing does not silently reset it. `ProductDetailData.options` is untouched — the data is server truth and still typed, so the control is a component away once the crash is diagnosed. Three candidates, all base's and all reachable only once `dwopt_` is in the URL: `getCurrentOptionModel`'s unchecked `filter(...)[0].selectedValueId`, a `getOptionValue` miss feeding `setSelectedOptionValue(option, null)`, and `getOptions`' unchecked `getSelectedOptionValue(option).ID`. Deferred: the chooser dialog the returned offer opens (2.11) |
...
| 4.5 | Account-SavePassword | JSON | `useSavePassword` | DONE | `server.append` with its own `route:BeforeComplete` → `FormResultData`, as 3.4 and 4.3 do. Base checks both halves in one call — `setPassword(new, current, true)` — then reads the failure apart: a new password the site's policy refuses lands on the new field, otherwise the current one was wrong. That distinction is the whole usefulness of the error, and it is base's; the port keeps it per field |
...
| # | Endpoint | Kind | Page / Hook | Status | Notes |
|---|---|---|---|---|---|
| 5.1 | Address-List | PAGE | `Address/List` | DONE | `server.append` → `addresses`, a hairline grid of `AddressData` (the DTO 4.1 built, now carrying `uuid` and `isDefault`). Base decided which entry was the default *positionally*, comparing every row against `addressBook[0]`; the platform does return the preferred address first so the answer was right, but the fact is named here — compared against the book's actual preferred address — rather than inferred from an index. Base's delete confirmation kept, on the same `ui/alert-dialog` the bag uses. The dashboard's address panel gains the two links 4.1 deferred. Dropped: the `actionUrls` bag (generated helpers) and the breadcrumb trail, whose two links the header wordmark and the page's own "Back to account" already are |
| 5.2 | Address-AddAddress | PAGE | `Address/Edit` | DONE | `server.append` → the empty `address` form as `AddressFormData`, on the page it shares with 5.3, exactly as base shared one template. Its two option fields — country, and the state list the definition includes from the shared `states` form — are the first real use of `FormFieldData.options`: the selects are built from the merchant's own lists, never from an array written into the frontend |
| 5.3 | Address-EditAddress | PAGE | `Address/Edit` | DONE | `server.append` → the same page, filled. Base clears the form and `copyFrom`s the address model, so the prefill rides on the fields themselves and nothing is copied across a second time here. `addressId` travels as a prop *and* as a field, because they mean different things: the prop is which address is being edited, the field is what the shopper may rename it to — and Address-SaveAddress needs both to tell a rename from a collision |
| 5.4 | Address-SaveAddress | JSON | `useSaveAddress` | DONE | `Address.js` repaired (the fatal `inertiaMiddleware` require and its three `setViewData({template, props})` appends gone; the pages come back as 5.1–5.3). `server.append` with its own `route:BeforeComplete` → `FormResultData` — the shape formerly called `AuthResultData`, renamed here because the address book is the eighth route answering it and "auth" had stopped being true. Base does create and edit through one route, told apart by whether the URL names an `addressId`, and its ID uniqueness check ("that ID already exists", on the field itself) is kept — the ID is the shopper's own label for the address, not a generated key |
| 5.5 | Address-DeleteAddress | JSON | `useDeleteAddress` | DONE | `server.append` → `AddressDeletedData`. Base's three fields were DOM bookkeeping — the UUID of the row to delete, the word "Default" to re-label whichever address inherited it, and a line for an emptied book — and a re-render has no rows to reconcile; what survives is the count base computed and threw away. Its default-promotion (removing the default promotes the first remaining address) and account-edited email are kept. Adds the `refusedSignedOut` seam: base's `validateLoggedInAjax` answers a 500 the browser rejects on before reading the body, so a shopper whose session ended would see "Request failed with status code 500" instead of being sent to sign in — normalized to the envelope, whose `redirectUrl` the client already follows. Left standing: base reads `getAddress(addressId).getUUID()` unchecked, so a hand-made URL naming an address that is not there throws |
| 5.6 | Address-SetDefault | N/A | — | DONE | Confirmed no UI surface: it promotes one address to preferred and redirects back to Address-List. No adapter help needed — the route is not Inertia-enabled, so its 302 is followed by the visit's own XHR and the now-ported list answers as a page. What the row ships is the control base put on every non-default card, as a button that visits rather than a link: base wrote this state change as a GET, and this storefront's `Link` prefetches on hover — a hover must not reorder someone's address book |
| 5.7 | PaymentInstruments-List | PAGE | `PaymentInstruments/List` | DONE | `server.append` → the wallet as `PaymentCardData`, the DTO 4.1 built for the dashboard's one-card glance, here as the whole list. Base's delete confirmation kept on `ui/alert-dialog`; the dashboard's payment panel gains the two links 4.1 deferred. Dropped: the two action URLs (generated helpers), the `noSavedPayments` flag (an empty array says it), the breadcrumbs, and base's `cardTypeImage` — a bundled brand SVG per issuer, chromatic where this world is not, so the issuer is named in words |
| 5.8 | PaymentInstruments-AddPayment | PAGE | `PaymentInstruments/Add` | DONE | `server.append` → `CreditCardFormData`. Base computes ten expiry years in the controller and hands them to the template *beside* the form, because the definition leaves that field's options empty; they are merged onto the field here, so the component sees one select with its choices like every other option field. `cardType` is read from the number as it is typed and posted in base's own hidden field — base ran cleave.js for that, and the port keeps its exact type names, since they are the IDs the platform knows the cards by, not labels. Dropped from the definition, each for a reason base gives: `securityCode` (a checkout-time value, not a stored one), `editNumber` and `paymentMethod` (plumbing for a radio group with one option — a choice of one is not a choice, so the form states it), and `saveCard` (checkout's question, not this page's) |
| 5.9 | PaymentInstruments-SavePayment | JSON | `useSavePayment` | DONE | `PaymentInstruments.js` repaired (fatal `inertiaMiddleware` require and its two `setViewData({template, props})` appends gone; the pages return as 5.7/5.8). `server.append` with its own `route:BeforeComplete` → `FormResultData`. None of base's work is reimplemented: it verifies the card against the methods the site actually accepts for this customer and country, has the platform validate number and expiry, creates the wallet entry, and calls the processor's `createToken` hook so the raw number is replaced by a token. RefArch's `basic_credit` processor ships that hook, so this row is not blocked by the missing payment processor — wave 6's authorization rows still are. Base's refusals are already per field (an unusable card type on the number, an expired date on the month) and stay there |
| 5.10 | PaymentInstruments-DeletePayment | JSON | `useDeletePayment` | DONE | `server.append` → `PaymentDeletedData`, the wallet's counterpart to 5.5's shape and for the same reason: base's answer was the UUID of a row to delete plus a line for an emptied wallet, and a re-render has no rows to reconcile. Base's transaction and account-edited email kept, and the same signed-out normalization 5.5 introduced. Left standing: base reads `paymentToDelete.raw` without checking the UUID matched anything, so a hand-made URL naming a card that is not in the wallet throws |
| 5.11 | Address-Header | N/A | — | DONE | Confirmed no UI surface, and in base not even a working one: it renders `account/header-anon` or `account/header-logged`, and **neither template exists** in app_storefront_base — the route throws if anything calls it, and nothing does (no `isinclude` anywhere in base, the plugins, or the port names it; only Account-Header is included, by the three page headers). Dead code inherited from an earlier SFRA layout. The fact it meant to carry — who is signed in — is the `auth.user` shared prop, rendered by `AccountMenu` (3.2/4.1/4.6) |
| 5.12 | PaymentInstruments-Header | N/A | — | DONE | Confirmed no UI surface: a second remote include of the very same `account/header` template Account-Header renders, with the same one fact (the customer's first name) — and, like 5.11, with no caller anywhere. Two routes for one fragment, neither of them reached; the `auth.user` shared prop replaced all three |
...
| # | Endpoint | Kind | Page / Hook | Status | Notes |
|---|---|---|---|---|---|
| 6.1 | Checkout-Begin | PAGE | `Checkout/Begin` | DONE | `Checkout.js` repaired; `server.append` → the whole flow as one page, four stages driven by base's own `stage` parameter. Everything base does before rendering is kept: the empty/invalid basket redirect to the bag, the default address copied onto shipment and billing, `ensureNoEmptyShipments`, the currency revalidation, the recalculation, and the fast-forward past the customer stage for a shopper who is signed in or has already given an email. The stage stays in the URL because it is the one piece of checkout state worth linking to and reloading on, and each stage advances with a visit, so a stage always opens on the basket as the server has it. `CheckoutFormsData` is four of base's forms in one prop — and mostly *includes* the port already typed: both address forms are the `address` form (`AddressFormData`, from 5.2) and the card is the `creditCard` form (`CreditCardFormData`, from 5.8), so checkout declared almost nothing new. Dropped: `reportingURLs` (10.4), `oAuthReentryEndpoint` (the customer stage links to Login-Show with base's own `rurl` index rather than re-hosting the OAuth buttons), and the account model beyond the two lists the selectors need. Base tabbed guest and sign-in; at this point in a purchase the shopper knows which they are, so both stand open |
| 6.2 | CheckoutServices-Get | JSON | `useCheckout` | DONE | `server.append` → `CheckoutResultData`, the one shape all eleven checkout routes answer with — base re-renders its whole OrderModel from every one of them because each can move the totals, which is `CartData`'s situation one stage on. The lines and the totals *are* the cart wave's DTOs (`CartLineItemData`, `CartTotalsData`) reading the same models, so the bag and the checkout print the same numbers from the same code. Base built this route for multi-ship; typed, it is simply "re-read the checkout". Its `allValid` check survives as `order.shippable` |
| 6.3 | CheckoutServices-SubmitCustomer | JSON | `useSubmitCustomer` | DONE | `server.append` with its own `route:BeforeComplete` → `CheckoutResultData`. Base reports field errors as an *array* of maps (it validates several forms per request); `answerCheckout` flattens it, losing nothing, since the keys are already the fields' full HTML names |
| 6.4 | CheckoutServices-LoginCustomer | JSON | `useCheckoutLogin` | DONE | `server.append` → `CheckoutResultData`, with base's redirect to the shipping stage carried as `redirectUrl`. Dropped: the re-issued `csrfToken` base puts in the answer because the session transforms on login — the port's token is an `always()` shared prop, so the visit that follows the redirect already carries a fresh one, and a token in a JSON body would be a second source of truth for it. A wrong password is base's `customerErrorMessage`, which belongs above the two fields rather than on either |
| 6.5 | CheckoutShippingServices-SubmitShipping | JSON | `useSubmitShipping` | DONE | `server.append` → `CheckoutResultData`. Base validates the address, writes it onto the shipment, applies the chosen method, recalculates, and copies it onto billing when "use as billing" is ticked — all kept, and the moved totals ride back with the answer. The method field is typed (`forms.shippingMethodId`) rather than named by hand: it belongs to the shipping form itself, not to the address group included into it |
| 6.6 | CheckoutShippingServices-SelectShippingMethod | JSON | `useSelectCheckoutShipping` | DONE | `server.append` → `CheckoutResultData`; the checkout counterpart of Cart-SelectShippingMethod (2.13), made where the shipping and tax totals are actually settled. Base applies the method the moment it is picked and recalculates, so the stage does too. **Two traps, both base's:** this route and 6.7 do *not* read the form definition — they read `firstName`, `address1`, `stateCode`, `countryCode`, `phone` straight off the request (`shippingHelpers.getAddressFromRequest`) while 6.5 reads the `dwfrm_` names, two conventions on the same data; and both *write* every one of those onto the shipment, nulling whatever is absent — so a call carrying only the method silently erases the shipping address. Both calls now send the whole address under the plain names |
| 6.7 | CheckoutShippingServices-UpdateShippingMethodsList | JSON | `useShippingMethods` | DONE | `server.append` → `CheckoutResultData`. What can ship to a postcode is a question only the server can answer, so base asks it as the address takes shape and the port does the same — choosing a country or state refreshes the method list and the totals it implies |
| 6.8 | CheckoutShippingServices-ToggleMultiShip | JSON | — | DONE | Typed with no hook and no surface, the outcome 2.5 and 2.15 established. The port's basket is single-shipment (2.1): base offers this only when the site enables multi-ship *and* the basket has more than one line, and the UI behind it is a per-line address assignment the port does not render. The route answers the same typed order as every other checkout route, so a multi-ship surface would find it ready |
| 6.9 | CheckoutAddressServices-CreateNewAddress | JSON | — | DONE | Multi-ship only — base invokes it when a shopper assigns one product to a second address — so it is typed without a hook, as 6.8 is. Base's work (create the shipment, move the line onto it, give it a method, recalculate) is untouched, and the new shipment's UUID is in the typed order as `shipping[].uuid` |
| 6.10 | CheckoutAddressServices-AddNewAddress | JSON | — | DONE | "Only In multiShip", in base's own words; typed without a hook for the same reason as 6.8/6.9. Its field errors are the shipping address form's, which `answerCheckout` already lands per field, so a multi-ship surface would render them exactly as the single-shipment stage renders its own |
| 6.11 | CheckoutServices-SubmitPayment | JSON | `useSubmitPayment` | DONE | **Not blocked after all.** The row was expected to need a payment processor; RefArch ships one — `basic_credit` — and it needs no gateway: `Handle` asks the platform whether the number, type and expiry are usable, and `Authorize` writes a transaction ID without anything leaving the instance. So `server.append` → `CheckoutResultData`, with base's validation, its `Handle` hook and its payment instrument all kept. What genuinely cannot be done here is *exercising* it: the sandbox is retired, so the row is ported and unverified, not blocked. A saved card pays by `storedPaymentUUID` and asks only for the security code, since the number the wallet holds is a token |
| 6.12 | CheckoutServices-PlaceOrder | JSON | `usePlaceOrder` | DONE | `server.append` → `PlacedOrderData`: the order number, the token that lets a guest see it without an account, and base's own continue URL — which the review stage POSTs to, as base did (the first cut of this got it wrong; see 7.1). The one checkout call that does not answer with a basket, because after it there is no basket. Base's dozen refusals — an unavailable product, an invalid address, a declined card, a technical failure — are each `{error, errorMessage}` and all become the envelope, so the button reports what actually happened instead of a generic failure. Base's fraud hooks, confirmation email and address-book harvesting are untouched |
...
| # | Endpoint | Kind | Page / Hook | Status | Notes |
|---|---|---|---|---|---|
| 7.1 | Order-Confirm | PAGE | `Order/Confirm` | DONE | Ported out of order, to fix a defect 6.12 shipped: the review stage handed off to Order-Confirm as a GET with `?ID=&token=`, and Order-Confirm is a POST reading `req.form.orderID` and `req.form.orderToken` — behind `Order.js`, which was still one of the fatal controllers. So placing an order *succeeded* and then 500'd on the way to the confirmation: the worst kind of failure, since the order exists and the shopper is shown an error. `Order.js` repaired; `server.append` → `OrderConfirmationData`, which is `CheckoutOrderData` (a confirmation is the basket that just became an order, from the same OrderModel) plus the three facts only an order has. The handoff is now `router.post` with base's own field names, so the order number and token stay out of history and referrers. **Repair**: base's double-render guard compared its remembered order against `req.querystring.ID` on a route that reads everything else off `req.form`, so it compared against `undefined` and then stored `undefined` — the guard could never fire. Both sides now read `req.form.orderID`. Deferred: the guest's "turn this into an account" password form is row 7.6; `returningCustomer` already says which shopper is looking |
